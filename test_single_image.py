...
parser.add_argument("--anchor_path", type=str, default="./data/voc_anchors.txt",
...
#                     help="The path of the weights to restore.")
parser.add_argument("--restore_path", type=str, default="./kmeans_checkpoint/kmeans_prune_restore_model_all.ckpt",
                    help="The path of the weights to restore.")
...
with tf.Session(config=config) as sess:
    ...
    sliming_yolo_model = sliming_yolov3(args.num_class, args.anchors)
    with tf.variable_scope('yolov3'):
        pred_feature_maps = sliming_yolo_model.forward_include_res_with_prune_factor(input_data, prune_factor=0.8)
    pred_boxes, pred_confs, pred_probs = sliming_yolo_model.predict(pred_feature_maps)
    ...
    file_dir = '/home/lovepan/work/YOLOv3_TensorFlow_myself/'
    for img in file_list:
        img_ori = cv2.imread(file_dir + img)
