...
def evaluate_on_gpu(sess, gpu_nms_op, pred_boxes_flag, pred_scores_flag, y_pred, y_true, num_classes, iou_thresh=0.5, calc_now=True):
    ...
    for i in range(num_images):
        ...
        pred_confs = y_pred[1][i:i + 1]
        pred_probs = y_pred[2][i:i + 1]
        pred_sigma = y_pred[3][i:i + 1]
        # print("current pred sigma is", pred_sigma)
        # print(np.shape(pred_sigma))

        mean_pred_sigma = np.mean(pred_sigma, axis=-1, keepdims=True)
        # mean_pred_sigma = np.
        # print("mean pred sigma is", mean_pred_sigma)
        # print(np.shape(mean_pred_sigma))
        # pred_boxes: [N, 4]
        # pred_confs: [N]
        # pred_labels: [N]
        # pred_sigma: [N, 4]
        ...
        pred_boxes, pred_confs, pred_labels = sess.run(gpu_nms_op,
                                                       feed_dict={pred_boxes_flag: pred_boxes,
                                                                  pred_scores_flag: pred_confs * pred_probs*(mean_pred_sigma)})
        # len: N
        pred_labels_list = [] if pred_labels is None else pred_labels.tolist()
        if pred_labels_list == []:
            continue
...
def get_preds_gpu(sess, gpu_nms_op, pred_boxes_flag, pred_scores_flag, image_ids, y_pred):
    ...
    boxes, scores, labels = sess.run(gpu_nms_op,
                                     feed_dict={pred_boxes_flag: pred_boxes,
                                                pred_scores_flag: pred_confs * pred_probs*(1- np.mean(pred_sigma, axis=-1, keepdims=True)) })
