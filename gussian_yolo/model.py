...
class yolov3(object):
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm], reuse=reuse):
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('yolov3_head'):
                    ...
                    feature_map_1 = slim.conv2d(net, 3 * (9 + self.class_num), 1,
                    ...
                    feature_map_2 = slim.conv2d(net, 3 * (9 + self.class_num), 1,
                                                stride=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer())
                    ...
                    feature_map_3 = slim.conv2d(feature_map_3, 3 * (9 + self.class_num), 1,
    ...
    def reorg_layer(self, feature_map, anchors):
        ...
        # conf_logits: [N, 13, 13, 3, 1]
        # prob_logits: [N, 13, 13, 3, class_num]
        return x_y_offset, boxes, conf_logits, prob_logits, y_pred_sigma
    ...
    def predict(self, feature_maps):
        ...
        def _reshape(result):
            x_y_offset, boxes, conf_logits, prob_logits, y_pred_sigma = result
            ...
            # conf_logits: [N, 13*13*3, 1]
            # prob_logits: [N, 13*13*3, class_num]
            return boxes, conf_logits, prob_logits, y_pred_sigma

        boxes_list, confs_list, probs_list, pred_sigma = [], [], [], []
        for result in reorg_results:
            boxes, conf_logits, prob_logits, y_pred_sigma= _reshape(result)
            confs = tf.sigmoid(conf_logits)
            probs = tf.sigmoid(prob_logits)
            # y_pred_sigma = tf.sigmoid(y_pred_sigma)
            boxes_list.append(boxes)
            confs_list.append(confs)
            probs_list.append(probs)
            pred_sigma.append(y_pred_sigma)
        ...
        y_pred_sigma = tf.concat(pred_sigma, axis=1)
        ...
        return boxes, confs, probs, y_pred_sigma
    ...
    def loss_layer(self, feature_map_i, y_true, anchors):
        ...
        x_y_offset, pred_boxes, pred_conf_logits, pred_prob_logits, y_pred_sigma = self.reorg_layer(feature_map_i, anchors)
...
#
# ####################################### guassian loss ##############################################
        y_true_box = tf.concat([true_xy, true_tw_th], axis=-1)
        y_pred_mu = tf.concat([pred_xy, pred_tw_th], axis=-1)
        x_loss = self.nll_loss(y_true_box[..., 0:1], y_pred_mu[..., 0:1], y_pred_sigma[..., 0:1])
        x_loss = object_mask * box_loss_scale * x_loss
        y_loss = self.nll_loss(y_true_box[..., 1:2], y_pred_mu[..., 1:2], y_pred_sigma[..., 1:2])
        y_loss = object_mask * box_loss_scale * y_loss
        w_loss = self.nll_loss(y_true_box[..., 2:3], y_pred_mu[..., 2:3], y_pred_sigma[..., 2:3])
        w_loss = object_mask * box_loss_scale * w_loss
        h_loss = self.nll_loss(y_true_box[..., 3:4], y_pred_mu[..., 3:4], y_pred_sigma[..., 3:4])
...
######################################################################################################
        ...
        return gussian_loss, wh_loss, conf_loss, class_loss
    ...
    def compute_loss(self, y_pred, y_true):
        ...
        '''
        loss_xy, loss_wh, loss_conf, loss_class, loss_gussian_2 = 0., 0., 0., 0., 0.
        ...
        ################# gussain loss #########################
        total_loss = loss_gussian_2  + loss_conf + loss_class
        ########################################################
