...
from utils.eval_utils import evaluate_on_cpu, evaluate_on_gpu, get_preds_gpu_asff, voc_eval, parse_gt_rec
from utils.nms_utils import gpu_nms

from gussian_yolo.guasian_model import yolov3
...
a = tf.print(tf.shape(pred_feature_maps))
...
with tf.Session() as sess:
    ...
    for epoch in range(args.total_epoches):
        ...
        for i in trange(args.train_batch_num):
            _, summary, __y_pred, __y_true, __loss, __global_step, __lr, b = sess.run(
                [train_op, merged, y_pred, y_true, loss, global_step, learning_rate, a],
        ...
        if epoch % args.val_evaluation_epoch == 0 and epoch >= args.warm_up_epoch:
            ...
            for j in trange(args.val_img_cnt):
                ...
                pred_content = get_preds_gpu_asff(sess, gpu_nms_op, pred_boxes_flag, pred_scores_flag, __image_ids, __y_pred)
