...
import sft_yolo.sfg_args as args
...
############################## fourth prune #################################################################################
# with tf.variable_scope('yolov3'):
#     pred_feature_maps = yolo_model.forward_include_res_with_prune_factor(image, prune_factor=0.8, is_training=is_training, prune_cnt=4)
############################################################################################################################
...
for var in tf.global_variables():
    ...
    try:
        ...
        idx = 0
    if ('darknet' in current_layer_type and idx in prune_darknet_layer and 'weights' in current_layer_type) or ('yolov3_head' in current_layer_type and idx in prune_head_layer and 'weights' in current_layer_type):
...
with tf.Session() as sess:
    ...
    best_mAP = -np.Inf
    from sft_yolo.sfp_prun import soft_prun

    sfp_model = soft_prun(sess=sess, pruning_factor=0.8, prune_iter_cnt=1)
    soft_weights = {}
    ...
    for epoch in range(args.total_epoches):
        ...
        ########################## sft prun ####################################
        # print("prun layer tensor is ", tensor_list)
        for i, tensor in enumerate(tensor_list):
            # print(tensor_list[i])
            tensor_list[i] = tf.py_func(sfp_model.sfp_prun_with_zero_from_sess_py, inp=[tensor_list[i]], Tout=[tf.float32])
        print("end soft prune")
        ...
        for i in trange(args.train_batch_num):
            ...
            if __global_step % args.train_evaluation_step == 0 and __global_step > 0:
                ...
                recall, precision = evaluate_on_gpu(sess, gpu_nms_op, pred_boxes_flag, pred_scores_flag, __y_pred,
                                                    __y_true, args.class_num, args.eval_threshold)
        ...
        ##### soft prun must save model ################################
        # saver_to_save.save(sess, args.save_dir + 'soft_temp_model')
        ...
        if epoch % args.val_evaluation_epoch == 0 and epoch > 0:
            ...
            if mAP > best_mAP:
                best_mAP = mAP
                saver_best.save(sess, '/home/pcl/tensorflow1.12/my_github/YOLOv3_TensorFlow/sft_yolo/soft_prun_checkpoint/' + 'best_model_Epoch_{}_step_{}_mAP_{:.4f}_loss_{:.4f}_lr_{:.7g}'.format(
                                    epoch, int(__global_step), best_mAP, val_loss_total.average, __lr))

                print("begin hard prune.......")
                new_checkpoint = '/home/pcl/tensorflow1.12/my_github/YOLOv3_TensorFlow/sft_yolo/soft_prun_checkpoint/' + 'best_model_Epoch_{}_step_{}_mAP_{:.4f}_loss_{:.4f}_lr_{:.7g}'.format(epoch, int(__global_step), best_mAP, val_loss_total.average, __lr)
                new_hard_prun_dict = {}
                new_hard_prun_dict = sfp_model.hard_prun_without_zero(new_checkpoint)
                sfp_model._reconstruction_model(new_hard_prun_dict)
                print("end hard prune")
